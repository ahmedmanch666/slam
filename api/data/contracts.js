...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            await db.execute({
                sql: `INSERT INTO contracts (id, user_id, company_id, tender_id, title, status, value, start_date, end_date, notes) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                args: [id, userId, company_id || null, tender_id || null, title, status || null, value || null, start_date || null, end_date || null, notes || null]
            });
        ...
        if (req.method === 'PUT') {
            ...
            await db.execute({
                sql: `UPDATE contracts SET company_id = ?, tender_id = ?, title = ?, status = ?, value = ?, start_date = ?, end_date = ?, notes = ? 
                      WHERE id = ? AND user_id = ?`,
                args: [company_id || null, tender_id || null, title, status || null, value || null, start_date || null, end_date || null, notes || null, id, userId]
            });
        ...
        if (req.method === 'DELETE') {
            ...
            await db.execute({
                sql: 'DELETE FROM contracts WHERE id = ? AND user_id = ?',
                args: [body.id, userId]
            });
