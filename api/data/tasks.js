...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'PUT') {
            ...
            await db.execute({
                sql: `UPDATE tasks SET related_type = ?, related_id = ?, title = ?, priority = ?, status = ?, due_date = ?, notes = ? 
                      WHERE id = ? AND user_id = ?`,
                args: [related_type || null, related_id || null, title, priority || null, status || null, due_date || null, notes || null, id, userId]
            });
